...
const getMenuElementsAndValidateOpenMenu = (
  getByText: any,
  getByRole: any,
  getByTestId: any
) => {
...
it('shows first section as active when at the root path', () => {
  ...
  const activeSection = getClosestAnchorAndValidateExists(
    getByText,
    /Active Section/i
  );
  const otherSection = getClosestAnchorAndValidateExists(
    getByText,
    /Another Section/i
  );
...
it('opens and closes menu when menu clicked', () => {
  ...
  const { overlay, nav } = getMenuElementsAndValidateOpenMenu(
    getByText,
    getByRole,
    getByTestId
  );
